...
class protexiom extends eqLogic {
  ...
  public static $_widgetPossibility = array('custom' => array(
    ...
    public static function pull() {
      $t0 = microtime(true);
      log::add(__CLASS__, 'debug', '[*-*] '.getmypid().' Starting Protexiom pull '.date("Y-m-d H:i:s"));
      foreach (eqLogic::byType(__CLASS__,true) as $protexiom) {
        ...
        if (!($protexiom->_spBrowser->authCookie)) { //Empty authCookie mean not logged in
          if($myError=$protexiom->_spBrowser->doLogin()) {
            ...
            $protexiom->log('debug', 'Successfull login during scheduled pull. authCookie cached.');
          }
        }
    ...
    public static function deamon_start($_debug = false) {
      ...
      if ($deamon_info['launchable'] == 'ok') {
        ...
        $cron->run();
        log::add(__CLASS__, 'debug', '[*-*] '.getmypid().' Daemon started (launchable='.$deamon_info['launchable'].')');
      }else{
        log::add(__CLASS__, 'debug', '[*-*] '.getmypid().' Daemon unlaunchable. Won\'t start (launchable='.$deamon_info['launchable'].')');
      }
    ...
    /**
     * Get plugin version from the plugin info.json file
     * @author Fdp1
     * @return (string) pluginVersion
     */
    public static function getPluginVersion() {
      $infoJsonPath = plugin::getPathById(__CLASS__);
...
// message::add(__CLASS__,json_encode($status));
        // {"ZONE_A":"off","ZONE_B":"off","ZONE_C":"off","BATTERY":"ok","LINK":"ok","DOOR":"ok","ALARM":"ok","TAMPERED":"ok","GSM_LINK":"GSM connect\u00c3\u00a9 au r\u00c3\u00a9seau","GSM_SIGNAL":"4","GSM_OPERATOR":"Orange","CAMERA":"disabled","LastRefresh":"2022-02-05 19:48:24"}
        if($status['ALARM']==""){
...
// $this->log('debug', "Status: " .$status[$cmd->getConfiguration('somfyCmd')] ." Command [$currentLogicalId] subType: $subType NewValue: $newValue");
      ...
      if($batteryStatusChanged) {
        if($status['BATTERY'] == "ok") $newValue = '100';
        else $newValue = '10';
    ...
    public function setElementsFromSpBrowser() {
      ...
      foreach ($elements as $elementId => $element) {
        ...
        if ( ! is_object($eqLogic) ) {
          ...
          $eqLogic->save();
          @message::add(__CLASS__, 'New protexiom subdevice created: '.$this->name.'/'.$eqLogic->getName(), '', $this->name);
